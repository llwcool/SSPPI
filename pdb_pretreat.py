...
in_field = sys.argv[1]
if "-" not in in_field:
	in_fields = in_field.split('/')[-1].split(".")[0].split('_')
	pdb_id = in_fields[0]
	chain_ids1 = in_fields[1]
	pdb_filename = "/home/llw/BSSPPI/data_preparation/raw_pdbs/"+pdb_id+".pdb"
else:
	in_fields = in_field.split('/')[-1].split(".")[0].split('-')
	pdb_id = in_fields[1]
	chain_ids1 = "A"
	pdb_filename = "/home/llw/BSSPPI/data_preparation/raw_pdbs/"+in_field+".pdb"
...
if not os.path.exists("/home/llw/SSPPI_DeepPPISP/oridata"):
    os.makedirs("/home/llw/SSPPI_DeepPPISP/oridata")

fp1 = open("/home/llw/SSPPI_DeepPPISP/oridata/"+outfilename+".data","w+",encoding="utf-8")
...
if not os.path.exists(BSSPPIopts['ply_chain_dir']):
    os.makedirs(BSSPPIopts['ply_chain_dir'])
if not os.path.exists(BSSPPIopts['pdb_chain_dir']):
    os.makedirs(BSSPPIopts['pdb_chain_dir'])
shutil.copy(out_filename1+'.ply', BSSPPIopts['ply_chain_dir'])
shutil.copy(out_filename1+'.pdb', BSSPPIopts['pdb_chain_dir'])
