...
def start_train(dict_distance_length, dict_seq, dict_pdb, L, gpu, batch_size,\
          dis, info_train, info_eval, epo, cv_time):
    batch_size = int(batch_size)
    win_length = dict_distance_length[dis]
    ...
    train_dataset = dataset(train_sum, L, dict_seq, dict_pdb, dis, win_length)
    eval_dataset = dataset(eval_sum, L, dict_seq, dict_pdb, dis, win_length, False)
    resdir = f"work/result_{cv_time}"
    ...
    while j < epo:
        ...
        if retrain > 10 or lossNochange > 20:
            break
        auroc, state_dict_now, y_true, y_pred, train_res, eval_res = mytrain(train_dataset, eval_dataset, str(j), state_dict, batch_size, L, dis, win_length, resdir)
        if j == 0:
        ...
        if auroc >= auroc_best and eval_res[-1] > 0.05:
...
def mytrain(train_dataset, eval_dataset, j, state_dict_now, batch_size, L, dis, win_length, resdir):
    ...
    eval_loader = paddle.io.DataLoader(
        eval_dataset, batch_size=batch_size // 4, shuffle=False)
